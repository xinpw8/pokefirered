...
{
    bool8 (*func)(void) = (bool8 (*)(void))(uintptr_t)ScriptReadPtr(ctx);
...
{
    void (*func )(void) = ((void (*)(void))(uintptr_t)ScriptReadPtr(ctx));
    func();
    return FALSE;
}
...
bool8 ScrCmd_setvaddress(struct ScriptContext * ctx)
{
    ...
    uintptr_t addr2 = (uintptr_t)ScriptReadPtr(ctx);
...
{
    u8 condition = ScriptReadByte(ctx);
    const u8 * scrptr = ScriptReadPtr(ctx) - sAddressOffset;
    if (sScriptConditionTable[condition][ctx->comparisonResult] == 1)
        ScriptJump(ctx, scrptr);
    return FALSE;
}
...
bool8 ScrCmd_loadword(struct ScriptContext * ctx)
{
    ...
    ctx->data[index] = (uintptr_t)ScriptReadPtr(ctx);
#else
...
{
    u8 * dest = (u8 *)ScriptReadPtr(ctx);
    *dest = *ScriptReadPtr(ctx);
    return FALSE;
}
...
{
    const u8 value1 = ctx->data[ScriptReadByte(ctx)];
    const u8 value2 = *ScriptReadPtr(ctx);
...
{
    const u8 value1 = *ScriptReadPtr(ctx);
    const u8 value2 = ctx->data[ScriptReadByte(ctx)];
...
bool8 ScrCmd_applymovement(struct ScriptContext * ctx)
{
    ...
    const void *movementScript = ScriptReadPtr(ctx);
...
{
    const u8 *msg = ScriptReadPtr(ctx);
...
{
    const u8 *msg = (const u8 *)((uintptr_t)ScriptReadPtr(ctx) - sAddressOffset);

    ShowFieldMessage((u8 *)msg);
    return FALSE;
}
...
{
    u8 stringVarIndex = ScriptReadByte(ctx);
    const u8 *text = ScriptReadPtr(ctx);
...
{
    const u8 *ptr = (u8 *)((uintptr_t)ScriptReadPtr(ctx) - sAddressOffset);
...
{
    u8 stringVarIndex = ScriptReadByte(ctx);
    uintptr_t addr = (uintptr_t)ScriptReadPtr(ctx);
...
{
    const void *ptr = ScriptReadPtr(ctx);
